...
from todoBot.timeManip import now, todayIn, tomorrowIn
from todoBot import botPatch, apihelper, types, util
...
class TeleBot:
    ...
    def just_handler(self):
        res = list()
        curTime = now()
        table = botPatch.getTable()
        mark = 0        
    
        if table == []:
            print("There is no table yet")
            return 1
        ...
        for x in table:
            ...
            if mark == 0:
                if curTime >= x[4] and x[4] != 972532281307:
                    if x[3] != -1:
                        res.append([x[0], x[1], x[2], x[3], tomorrowIn(x[3])]) 

                    if (x[4] - curTime) <= 30:
                        self.send_message(x[0], "You have a task! : " + str(x[1]) )
                        self.send_message(x[0], "Description : " + str(x[2]) )         
                else:       
                    res.append(x) 
                    mark = 1             
            else:
                res.append(x)
        botPatch.write_table(res)
        return 0
